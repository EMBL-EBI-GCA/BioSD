...
my $sample_name = $sample->property('Sample Name')->value;
...
foreach my $group (@$groups) {
  print $group->id, ": ", $group->property('Submission Title')->value, "\n";
}
print "\n";
...
foreach my $ancester (@$derived_from) {
  print $ancester->id, ": ", $ancester->property('Sample Name')->value, "\n";
}
print "\n";
